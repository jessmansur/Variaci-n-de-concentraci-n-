...
st.markdown("""
    ...
    /* Contenedor para la imagen al final a la derecha */
...
# Función para formatear números según tu preferencia (1.234,56)
...
def main():
    ...
    # Cambio solicitado: Masa inicial -> Cantidad inicial
    d0_input = c_d1.number_input("Cantidad inicial", value=10.0, step=0.1, key="comp_inicial")
    ...
    st.sidebar.markdown('<p class="validation-hint">si no se carga concentración inicial, se requiere cargar masa inicial del compuesto (en proporción masa en masa)</p>', unsafe_allow_html=True)
    ...
    fe_val = cf1.number_input("Flujo Entrada", value=1.0, format="%.3f", step=0.001)
    ...
    fs_val = cf3.number_input("Flujo Salida", value=1.0, format="%.3f", step=0.001)
    ...
    ce_input = st.sidebar.number_input("Concentración Entrada", value=0.1, format="%.4f", step=0.0001)
    ...
    st.sidebar.markdown(f'<p class="unit-hint">SI: {fmt(T_max, 0)} s</p>', unsafe_allow_html=True)
    ...
    if abs(Fe_si - Fs_si) < 1e-7:
        st.markdown(f'<div class="pink-box">✅ <b>Estado Estacionario Detectado:</b> Los flujos son iguales. La masa total se mantiene constante en {fmt(M0)} kg.</div>', unsafe_allow_html=True)
    else:
        delta_m = M_t[-1] - M0
        st.markdown(f'<div class="pink-box">⚖️ <b>Sistema Dinámico:</b> La masa total {"aumentó" if delta_m > 0 else "disminuyó"} en {fmt(abs(delta_m))} kg durante el proceso.</div>', unsafe_allow_html=True)
    ...
    with g1:
        ...
        st.pyplot(fig1)
        st.metric(label=f"Concentración final (t={t_input})", value=f"{fmt(C_t[-1], 3)} kg/kg")
    ...
    with g2:
        ...
        st.pyplot(fig2)
        st.metric(label=f"Masa final del compuesto (t={t_input})", value=f"{fmt(D_t[-1], 3)} kg")

    # --- FOOTER CON IMAGEN ---
    # Colocamos la imagen al final, alineada a la derecha. 
    # Asegúrate de subir la imagen a tu repositorio de GitHub como 'footer_image.png'
